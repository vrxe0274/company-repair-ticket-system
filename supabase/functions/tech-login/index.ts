...
import {
  ...
  deriveTechKey, checkRateLimit, updateRateLimit, createSession,
...
Deno.serve(async (req) => {
  try {
    ...
    console.error('tech-login unhandled error:', err)
...
async function handleRequest(req: Request): Promise<Response> {
  ...
  const session = await createSession(supabase, {
    role: 'Technician', username: normalizedUsername, name: resolvedName, persistent: persistent ?? false,
  })
  return json(200, {
    ok: true,
    name:               resolvedName,
